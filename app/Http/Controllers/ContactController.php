	...
	public function store(Request $request) {
		try {
			...
			$contact = Contact::create([
				...
				'product_category_id' => $request->category_id ?? null
			]);

			// Store Images  for      	'image',
			$updates = [];
			...
			if ($request->hasFile('finger_print')) {
				$image = $request->file('finger_print');
				$imageName = time() . '_finger.' . $image->getClientOriginalExtension();
				$image->move(public_path('uploads/contacts'), $imageName);
				$updates['finger_print'] = 'uploads/contacts/' . $imageName;
			}
			...
			if ($request->hasFile('signature')) {
				...
				$image->move(public_path('uploads/contacts'), $imageName);
				$updates['signature'] = 'uploads/contacts/' . $imageName;
			}
			...
			return response()->json(['success' => true, 'message' => 'Contact created successfully', 'contact' => $contact], 201);
		} catch (\Exception $e) {
			if ($request->ajax()) {
